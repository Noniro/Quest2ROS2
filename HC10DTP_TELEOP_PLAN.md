...
- [ ] **Step 7: Connect to MotoROS2** on the real YRC1000micro cabinet.
  ...
  - Coexistence plan: put our cabinet on its own `ros_domain_id` (set in the cabinet's `motoros2_config.yaml`) or give it a node/topic namespace; the teleop node's `joint_states_topic` / `trajectory_topic` parameters can then target the namespaced topics so cross-robot wiring is impossible.
